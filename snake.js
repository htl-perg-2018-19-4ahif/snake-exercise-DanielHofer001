...
function game() {
   ...
   // checkCollision();
    ...
    if (gameOver) {
      ...
      //changed=true nicht mehr zeichnen
...
function generateApple() {
    //to avoid long waiting at game end round the decimal
...
function drawGameField() {
    ...
    drawHorizontalLine(1, height, width); changed=true; 
...
function LookForKeypress() {
    ...
    process.stdin.on('keypress', function (ch, key) {
        ...
        switch (key.name) {
            case "up": if (changesY != 1 && changesY!=-1) { changesX = 0; changesY = -1;  drawSnake();}; break;
            case "down": if (changesY != -1 && changesY != 1) { changesX = 0; changesY = 1; drawSnake();}; break;
            case "left": if (changesX != -1&&changesX != 1) { changesX = -1; changesY = 0;  drawSnake();}; break;
            case "right": if (changesX != 1 &&changesX != -1) { changesX = 1; changesY = 0;  drawSnake();}
    }
});
